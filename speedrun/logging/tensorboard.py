...
class TensorboardMixin(object):
    ...
    def logger(self):
        if tx is None:
            raise ModuleNotFoundError(
                "TensorboardMixin requires tensorboardX. You can "
    ...
    def log_scalar(self, tag, value, step=None):
        ...
        self.logger.add_scalar(
            tag=self.get_full_tag(tag), scalar_value=value, global_step=step
        )
        return self
    ...
    def log_image(self, tag, value, step=None):
        ...
        self.logger.add_image(
            tag=self.get_full_tag(tag), img_tensor=value, global_step=step
        )
        return self
    ...
    def log_histogram(self, tag, value, bins="tensorflow", step=None):
        ...
        self.logger.add_histogram(
            tag=self.get_full_tag(tag), values=value, bins=bins, global_step=step
        )
        return self
