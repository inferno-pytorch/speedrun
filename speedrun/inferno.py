...
try:
    from firelight.inferno_callback import (
        get_visualization_callback as firelight_visualizer,
    )
except ImportError:
...
class InfernoMixin(object):
    ...
    def trainer(self):
        ...
        if inferno is None:
            raise ModuleNotFoundError(
                "InfernoMixin requires inferno. You can "
    ...
    def inferno_build_optimizer(self):
        ...
        self._trainer.build_optimizer(
            self.get("trainer/optimizer"), **self.get("trainer/optimizer_kwargs")
        )
    ...
    def inferno_build_callbacks(self):
        ...
        if self.get("firelight") is not None:
            ...
            if firelight_visualizer is None:
                raise ImportError(
                    "firelight could not be imported but is present in the config file"
                )
            else:
